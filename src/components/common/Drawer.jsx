    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9998, display: 'flex' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1.5rem 1.25rem 1.25rem' }}>
          ...
          <button
            onClick={onClose}
            style={{ background: '#f5f5f5', border: 'none', width: 32, height: 32, borderRadius: '50%', cursor: 'pointer', fontSize: '0.85rem', color: '#666', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
          >✕</button>
        </div>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', padding: '0.5rem 0.75rem', flex: 1 }}>
          {items.map((item) => (
            ...
            >
              ...
              {item.badge && (
                <span style={{ fontSize: '0.65rem', fontWeight: 600, background: '#e8f5e9', color: '#1b4332', borderRadius: 980, padding: '0.15rem 0.5rem' }}>
                  {item.badge}
                </span>
              )}
            </button>
